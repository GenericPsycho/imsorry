...
export default async (client: Bot, interaction:Interaction) => {
	...
	if(interaction.isAnySelectMenu()) {
		...
		if (!inter) client.logger.warn(`${client.shard?.ids[0] ?? "Discord Client"}`, `No interaction found for Menu ${interaction.customId}`);
	}
	if(interaction.isChatInputCommand() || interaction.isContextMenuCommand()){
		inter = client.interactions.get(interaction.commandName.replace(/\s/g, "_"));
		if (!inter) client.logger.warn(`${client.shard?.ids[0] ?? "Discord Client"}`, `No interaction found for Command ${interaction.commandName}`);
	}
	if(interaction.isModalSubmit()) {
		inter = client.interactions.get(interaction.customId.split("|")[0]);
		if (!inter) client.logger.warn(`${client.shard?.ids[0] ?? "Discord Client"}`, `No interaction found for Modal ${interaction.customId}`);
	}
	if(!inter) return;
	try{
